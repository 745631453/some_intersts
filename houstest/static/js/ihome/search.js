...
function updateFilterDateDisplay() {
    ...
    if (startDate) {
        ...
        var new_sd = startDate
        var sd = location.search.split('&')[1].split('=')[1]
        var new_ed = endDate
        var ed = location.search.split('&')[2].split('=')[1]
        if (new_sd != sd | new_ed != ed) {
            var path = location.search.replace('ed=' + ed, 'ed=' + new_ed)
            var paths = path.replace('sd=' + sd, 'sd=' + new_sd)
...
$(document).ready(function () {
    ...
    $(".filter-item-bar>#asd").on("click", "input", function (e) {
        alert('111')
        if (!$(this).hasClass("active")) {
            $(this).addClass("active");
            $(this).siblings("input").removeClass("active");
            new_sd = $(this).attr('start-date')
            sd = location.search.split('&')[1].split('=')[1]
            new_ad = $(this).attr('end-date')
            ad = location.search.split('&')[2].split('=')[1]

            var path = location.search.replace('ad=' + ad, 'ad=' + new_ad, 'sd=' + sd, 'sd' + new_sd)
            location.href = path
        }
    })
    ...
    $(".filter-item-bar>.filter-sort").on("click", "li", function (e) {
        if (!$(this).hasClass("active")) {
            ...
            ss = $(this).attr('sort-key')
            var path = location.search.split('&sk=')[0] + '&sk=' + ss
            location.href = path
...
$(document).ready(function () {
    ...
    $.get('/house/allsearch/' + path, function (data) {
        if (data.code == '200') {
            var house_list = template('house_list', {houses: data.house_list})
            $('.house-list').append(house_list)

            var alist_list = template('alists_dir', {alists: data.alist})
            $('.filter-area').append(alist_list)

            var ar_list = template('ar_dir', {alists: data.ar})
            $('#qwer').html(ar_list)
